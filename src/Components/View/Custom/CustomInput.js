...
export default function CustomInput({
	...
	return (
		<FormControl sx={{ marginLeft: '10px', marginRight: '150px', ...sx }} error={error}>
			<InputLabel
				htmlFor={id}
				sx={{
					fontSize: '16px',
					...
					color: error ? 'error.main' : '#444', // Change color if there's an error
				}}
			>
				{label}
			</InputLabel>
			...
			<OutlinedInput
				...
				sx={{
					...
					'&.Mui-focused fieldset': {
						borderColor: error ? 'error.main' : '#3f51b5',
						borderWidth: '2px',
					},
					...sx, // Allow custom styles to be passed via props
				}}
